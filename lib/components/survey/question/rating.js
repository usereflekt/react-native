...
function SurveyRatingQuestion({ question, answer, onAnswer }) {
    ...
    // Derived colors
    const ratingInactive = (0, theme_1.withOpacity)(theme.colors.primary, 0.25);
    const borderColor = (0, theme_1.withOpacity)(theme.colors.primary, 0.12);
    const selectedBg = (0, theme_1.withOpacity)(theme.colors.primary, 0.02);
    ...
    const renderRatingStars = () => {
        ...
        for (let i = 1; i <= range; i++) {
          ...
          <react_native_svg_1.default width={size} height={size} viewBox="0 0 256 256">
            <react_native_svg_1.Path fill={isSelected ? theme.colors.primary : ratingInactive} d={isSelected
        ...
        return (<react_native_1.View style={[styles.ratingContainer, { gap }, !isLayoutReady && styles.hiddenWhileMeasuring]}>
        {stars}
      </react_native_1.View>);
    };
    const renderRatingNumbers = () => {
        ...
        for (let i = 1; i <= range; i++) {
          ...
          <react_native_1.Text style={[themedStyles.numberText, isSelected && themedStyles.numberTextSelected]}>{i}</react_native_1.Text>
        </react_native_1.Pressable>);
        }
        return <react_native_1.View style={themedStyles.numberRatingContainer}>{numbers}</react_native_1.View>;
    };
    ...
    return (<react_native_1.View style={questionStyles.questionContainer} onLayout={(event) => {
      ...
      {question.ratingConfig?.lowerLabel && question.ratingConfig?.upperLabel && (<react_native_1.View style={styles.ratingLabels}>
          <react_native_1.Text style={themedStyles.ratingLabel}>{question.ratingConfig.lowerLabel}</react_native_1.Text>
          <react_native_1.Text style={themedStyles.ratingLabel}>{question.ratingConfig.upperLabel}</react_native_1.Text>
        </react_native_1.View>)}
    </react_native_1.View>);
}
